...
def save(fname: str, Bezier: "Points[size=4 * (nops - 1)]") -> None:
    ...
    ---------
        fname   a string such that value of 'fname' + ".pnt" is a correct name
                of the file for saving Bezier control points
        Bezier  the array containing Bezier control points for saving
    Effect:
    -------
    ...
    """
    if exists(fname + ".pnt"):
        ans = input("Do you really want to rewrite file {}? [n | yes]:")
        if ans != 'yes':
            return
    file = open(fname + ".pnt", "w+")
    nops = Bezier.shape[0]
    for ic in range(nops):
        file.write(f"{Bezier[ic][0]}\t{Bezier[ic][1]}\t0.00\n")
    file.close()
